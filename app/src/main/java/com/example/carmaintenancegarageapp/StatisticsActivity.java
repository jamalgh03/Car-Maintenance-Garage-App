...
public class StatisticsActivity extends AppCompatActivity {
    ...
    private void exportReport() {
        // الحصول على مسار التخزين الخاص بالتطبيق لتصدير التقارير بصيغة PDF
        String pdfDirPath = getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS).getAbsolutePath() + "/CarMaintenanceReports/";
        String fileName = "StatisticsReport_" + System.currentTimeMillis() + ".pdf";

        // إنشاء الدليل إن لم يكن موجوداً
        File directory = new File(pdfDirPath);
        if (!directory.exists()) {
            boolean isCreated = directory.mkdirs();
            if (!isCreated) {
    ...
    private void fetchStatistics() {
        ...
        // إضافة سياسة إعادة المحاولة للتعامل مع وقت الانتظار
        jsonObjectRequest.setRetryPolicy(new DefaultRetryPolicy(
                30000, // مهلة 30 ثانية
                DefaultRetryPolicy.DEFAULT_MAX_RETRIES,
                DefaultRetryPolicy.DEFAULT_BACKOFF_MULT
    ...
    protected void onResume() {
        super.onResume();
        // تحديث الإحصائيات عند استئناف النشاط
        fetchStatistics();
    }
}
